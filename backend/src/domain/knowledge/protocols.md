...
- **Short metcon (7–20 min)**: AMRAP, EMOM, FOR_TIME — sustained effort, moderate volume.
- **Long aerobic (20+ min)**: AMRAP, CHIPPER, INTERVAL — pacing, higher total volume.
...
- **AMRAP**: Reps per round should allow 4–8+ rounds in the cap; avoid grinding singles.
- **FOR_TIME / 21-15-9**: Load and volume so target finish sits in intended time domain.
- **EMOM**: Work should fit in ~30–45 sec so rest is meaningful; heavy EMOM = fewer reps.
- **CHIPPER**: One-time rep ranges per movement; scale so athlete doesn’t blow up on one station.

## Key Rule

WODs are fetched and stored **to learn structure, scaling, and stimulus** — not to be randomly served. Generation must use this taxonomy to pick protocol and duration from **intended stimulus** (see stimulusEngine and methodology).
