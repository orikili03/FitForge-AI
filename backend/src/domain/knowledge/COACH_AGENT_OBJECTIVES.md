...
- **Do not randomly deliver WODs.** Use learned intelligence and athlete context to generate **tailored**, **principled** WODs via: `WorkoutEngine` → Assessment → Constraint → Progression → **ProgrammingAgent** (Groq or Simple).
...
| **programmingSystem.txt** | System prompt for GroqProgrammingAgent; can reference coach knowledge (methodology, protocols, scaling). |
