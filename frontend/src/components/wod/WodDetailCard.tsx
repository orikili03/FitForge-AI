...
export interface WodDetailCardWod {
  type: string;
  duration?: number;
  description: string;
  movements: string[];
        ...
        <WodBlock
          key={
            wod.movementItems?.length
              ? wod.movementItems.map((m) => m.name).join("\u0000")
              : wod.movements.join("\u0000")
          }
          type={wod.type}
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            {equipmentPresetName && (
              <span className="text-ds-body-sm text-ds-text-muted">
                Rig: <span className="font-medium text-ds-text">{equipmentPresetName}</span>
              </span>
            )}
